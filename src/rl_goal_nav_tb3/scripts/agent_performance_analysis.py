...
class PerformanceAnalyzer:
    ...
    def _compute_attention_weights(self, scan_data):
        # calculate attention weight based on proximity
        inverted = 1.0 / (scan_data + 0.1)
        return inverted / (inverted.sum() + 1e-8)
    
    def _compute_saliency_map(self, scan_data, position, goal_pos):
        # generated saliency map showing focus areas only
        # 2D saliency map
        grid_size = 50
        saliency = np.zeros((grid_size, grid_size))
        ...
        for i, (angle, dist) in enumerate(zip(angles, scan_data)):
            ...
            # For only nearby obstacles
            if dist < 3.5:  
                x = position[0] + dist * np.cos(angle)
                y = position[1] + dist * np.sin(angle)
                
                # Map to grid
                grid_x = int((x + 2.5) / 5.0 * grid_size)
                grid_y = int((y + 2.5) / 5.0 * grid_size)
                
                if 0 <= grid_x < grid_size and 0 <= grid_y < grid_size:
                    weight = 1.0 / (dist + 0.1)
                    saliency[grid_y, grid_x] += weight
        
        # Smoothing out saliency map (took a long time to figure out, used gaussian filter)
        saliency = gaussian_filter(saliency, sigma=2)
        return saliency
    
    # generated by gpt
    def _calculate_efficiency(self, trajectory, goal_pos):
        # generated by gpt
        if len(trajectory) < 2:
            return 0.0
        
        start_pos = trajectory[0]
        optimal_dist = np.linalg.norm(np.array(goal_pos) - np.array(start_pos))
        ...
        return optimal_dist / (actual_dist + 1e-6)
    ...
    def _calculate_jerk(self, trajectory):
        # logic for rate of change of acceleration(jerk)
        if len(trajectory) < 4:
            return 0.0
    ...
    def _calculate_min_clearance(self, trajectory, obstacles):
        # Minimum clearance from obstacles
        if not obstacles:
            return None
        
        min_clearance = float('inf') # use float
        for pos in trajectory:
            for obs in obstacles:
                dist = np.linalg.norm(np.array(pos) - obs.get_position()) - obs.radius
                min_clearance = min(min_clearance, dist)
        
        return min_clearance
    
    def _estimate_energy(self, trajectory):
        # Estimate energy consumption based on path
        if len(trajectory) < 2:
            return 0.0
        ...
        for i in range(1, len(trajectory)):
            ...
            # Simplified energy model [KISS principle in action]
            energy += dist ** 2  
    ...
    # TODO: create a logger and move code there
    def generate_comprehensive_report(self):
        ...
        print(f"Total Episodes Evaluated: {len(df)}")
        print("="*100)
        ...
        print(f"\n{' SUCCESS METRICS':<50}")
        print("-" * 100)
        print(f"  {'Overall Success Rate:':<40} {success_rate:>6.2f}%")
        print(f"  {'Successful Episodes:':<40} {df['success'].sum():>6d}")
        print(f"  {'Failed Episodes:':<40} {(~df['success']).sum():>6d}")
        print(f"  {'Success Rate (95% CI):':<40} [{self._confidence_interval(df['success'], 0.95)[0]*100:.2f}%, {self._confidence_interval(df['success'], 0.95)[1]*100:.2f}%]")
        
        # 2. EFFICIENCY METRICS
        print(f"\n{' EFFICIENCY & PERFORMANCE METRICS':<50}")
        print("-" * 100)
        print(f"  {'Average Path Efficiency:':<40} {df['efficiency'].mean():>6.3f} ± {df['efficiency'].std():.3f}")
        print(f"  {'Best Path Efficiency:':<40} {df['efficiency'].max():>6.3f}")
        print(f"  {'Worst Path Efficiency:':<40} {df['efficiency'].min():>6.3f}")
        print(f"  {'Median Path Efficiency:':<40} {df['efficiency'].median():>6.3f}")
        
        # 3. TIME & STEP METRICS
        print(f"\n{' TEMPORAL METRICS':<50}")
        print("-" * 100)
        print(f"  {'Average Steps to Goal:':<40} {df['steps'].mean():>6.1f} ± {df['steps'].std():.1f}")
        print(f"  {'Average Time to Goal:':<40} {df['time_to_goal'].mean():>6.2f}s ± {df['time_to_goal'].std():.2f}s")
        print(f"  {'Fastest Completion:':<40} {df['time_to_goal'].min():>6.2f}s")
        print(f"  {'Slowest Completion:':<40} {df['time_to_goal'].max():>6.2f}s")
        
        # 4. REWARD ANALYSIS
        print(f"\n{' REWARD METRICS':<50}")
        print("-" * 100)
        print(f"  {'Average Total Reward:':<40} {df['total_reward'].mean():>6.2f} ± {df['total_reward'].std():.2f}")
        print(f"  {'Best Episode Reward:':<40} {df['total_reward'].max():>6.2f}")
        print(f"  {'Worst Episode Reward:':<40} {df['total_reward'].min():>6.2f}")
        
        # 5. PATH QUALITY METRICS
        print(f"\n{'  PATH QUALITY METRICS':<50}")
        print("-" * 100)
        print(f"  {'Average Path Smoothness:':<40} {df['path_smoothness'].mean():>6.4f} ± {df['path_smoothness'].std():.4f}")
        print(f"  {'Smoothest Path:':<40} {df['path_smoothness'].min():>6.4f}")
        print(f"  {'Average Jerk:':<40} {df['jerk'].mean():>6.4f}")
        print(f"  {'Average Energy Consumption:':<40} {df['energy_consumption'].mean():>6.2f}")
        
        if df['clearance'].notna().any():
            print(f"  {'Minimum Obstacle Clearance:':<40} {df['clearance'].min():>6.3f}m")
            print(f"  {'Average Obstacle Clearance:':<40} {df['clearance'].mean():>6.3f}m")
        
        # 6. SUCCESS vs FAILURE COMPARISON
        if df['success'].any() and (~df['success']).any():
            success_df = df[df['success']]
            fail_df = df[~df['success']]
            
            print(f"\n{' COMPARATIVE ANALYSIS: SUCCESS vs FAILURE':<50}")
            print("-" * 100)
            print(f"  {'Metric':<40} {'Successful':>15} {'Failed':>15} {'Δ':>15}")
            print("-" * 100)
            print(f"  {'Average Steps:':<40} {success_df['steps'].mean():>15.1f} {fail_df['steps'].mean():>15.1f} {success_df['steps'].mean() - fail_df['steps'].mean():>15.1f}")
            print(f"  {'Average Efficiency:':<40} {success_df['efficiency'].mean():>15.3f} {fail_df['efficiency'].mean():>15.3f} {success_df['efficiency'].mean() - fail_df['efficiency'].mean():>15.3f}")
            print(f"  {'Average Reward:':<40} {success_df['total_reward'].mean():>15.2f} {fail_df['total_reward'].mean():>15.2f} {success_df['total_reward'].mean() - fail_df['total_reward'].mean():>15.2f}")
            print(f"  {'Average Smoothness:':<40} {success_df['path_smoothness'].mean():>15.4f} {fail_df['path_smoothness'].mean():>15.4f} {success_df['path_smoothness'].mean() - fail_df['path_smoothness'].mean():>15.4f}")
        
        # 7. STATISTICAL SUMMARY
        print(f"\n{' STATISTICAL SUMMARY':<50}")
        print("-" * 100)
        print(df[['efficiency', 'steps', 'total_reward', 'path_smoothness', 'jerk']].describe())
        
        print("\n" + "="*100)
        
        # NOTE: Converted numpy types to native Python types for JSON serialization
        ...
        report_data = {
            'metadata': {
                ...
                'analysis_version': '2.0_final_report'
            },
            'summary_statistics': {
                ...
                'avg_time': float(df['time_to_goal'].mean()),
                'avg_smoothness': float(df['path_smoothness'].mean()),
                'avg_jerk': float(df['jerk'].mean()),
                'avg_energy': float(df['energy_consumption'].mean())
            },
            'detailed_statistics': convert_to_native(df.describe().to_dict()),
            'episodes': convert_to_native(self.episode_data)
        }
        
        report_path = os.path.join(self.save_dir, 'comprehensive_report.json')
